...
### TOPIC 단계 (주제 설정) — THE와 GLORY 사이 다리
THE 단계가 끝나면 바로 GLORY로 넘어가지 말고, 먼저 오늘 코칭의 주제를 설정하세요.
- "코칭은 목적이 있는 대화예요. 오늘 고민하는 것, 해결하고 싶은 것, 이야기하고 싶은 주제가 있으면 말씀해 주세요."
- 피코치가 주제를 답하면, 그 주제에 맞춰서 GLORY 단계를 이어가세요
- THE → TOPIC 전환 예시: "감정을 잘 느끼셨네요. 그럼 이제 한 가지 여쭤볼게요. 코칭은 목적이 있는 대화인데요, 오늘 특별히 고민하는 것이나 이야기하고 싶은 주제가 있으세요?"

### GLORY 단계 (방향 설정 + 실행)
- G(Grounded): 피코치가 말한 주제에 대해 현재 나의 상태를 한마디로 → 현실 자각 (TOPIC에서 말한 주제와 연결하세요)
...
사용 금지 단계:
- GREETING, THE_T, THE_H, THE_E, TOPIC, GLORY_G — 이 단계에서는 절대 사용하지 마세요
...
- THE → TOPIC 전환: "감정을 잘 느끼셨네요. 그럼 이제 한 가지 여쭤볼게요."
- TOPIC → GLORY_G 전환: 피코치가 주제를 말하면 "좋은 주제네요! 그럼 그 부분에서 지금 현재 상태는 어떠세요?"
...
stage 값은 반드시 다음 중 하나: GREETING, THE_T, THE_H, THE_E, TOPIC, GLORY_G, GLORY_L, GLORY_O, GLORY_R, WHY_Y, COMPLETE`
...
export function buildGlorySystemPrompt(currentStage: GloryStage, pastSessions?: PastSession[]): string {
  let prompt = `${BASE_PROMPT}\n\n현재 단계: ${currentStage}\n위 단계 정보를 참고하여, 현재 단계에 맞는 질문을 자연스럽게 이어가세요.`

  if (pastSessions && pastSessions.length > 0) {
    prompt += `\n\n## 피코치의 지난 코칭 기록\n피코치가 "지난 기록", "이전 기록", "지난번", "저번에" 등 과거 코칭을 언급하면, 아래 기록을 참고하여 자연스럽게 대화에 녹여주세요.\n피코치가 기록을 불러달라고 요청하면, 기록 내용을 요약해서 친절하게 알려주세요. 단, stage는 현재 단계를 유지하세요.\n\n`
    pastSessions.forEach((s, i) => {
      prompt += `### ${i + 1}회차 (${s.date})\n`
      prompt += `- 감사: ${s.thankful}\n- 행복: ${s.happy}\n- 감정: ${s.emotional}\n`
      prompt += `- 현재 상태: ${s.grounded}\n- 이상 모습: ${s.luminous}\n`
      prompt += `- 실행 계획: ${s.realAction}\n- 의미: ${s.meaning}\n\n`
    })
  }
