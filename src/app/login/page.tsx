'use client'

import { useEffect } from 'react'
import { useRouter } from 'next/navigation'
import { signIn, useSession } from 'next-auth/react'
import { motion } from 'framer-motion'
import { MapArea } from '@/components/map/MapArea'
import { WorldKey } from '@/components/map/WorldTokens'

export default function LoginPage() {
  const router = useRouter()
  const { status } = useSession()

  useEffect(() => {
    if (status === 'authenticated') {
      router.push('/checkin')
    }
  }, [status, router])

  return (
    <main className="min-h-screen bg-slate-900 relative overflow-hidden">
      {/* 배경 맵 */}
      <div className="absolute inset-0 opacity-30 pointer-events-none">
        <MapArea
          selectedWorld={'cognition' as WorldKey}
          onNodeClick={() => {}}
          onNodeEnter={() => {}}
        />
      </div>

      {/* 상단 헤더 */}
      <header className="fixed top-0 left-0 right-0 z-40 bg-slate-900/80 backdrop-blur-lg border-b border-white/5">
        <div className="flex items-center justify-center px-4 py-3">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center">
              <span className="text-white font-bold text-sm">G</span>
            </div>
            <span className="text-white font-semibold">길로그</span>
          </div>
        </div>
      </header>

      {/* 로그인 폼 */}
      <div className="relative z-10 min-h-screen flex items-center justify-center px-4 pt-16">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="w-full max-w-sm"
        >
          <div className="bg-slate-800/90 backdrop-blur-xl rounded-2xl p-6 border border-white/10">
            <h1 className="text-2xl font-bold text-white text-center mb-2">길로그</h1>
            <p className="text-white/50 text-sm text-center mb-8">당신의 보이지 않는 애씀과 성장을 기록합니다</p>

            <div className="space-y-3">
              {/* 개인 사용자 카드 */}
              <button
                onClick={() => signIn('google', { callbackUrl: '/checkin' })}
                className="w-full py-4 px-4 rounded-xl bg-white text-gray-800 font-bold hover:bg-gray-100 transition-colors flex items-center gap-3"
              >
                <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24">
                  <path
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                    fill="#4285F4"
                  />
                  <path
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                    fill="#34A853"
                  />
                  <path
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                    fill="#FBBC05"
                  />
                  <path
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                    fill="#EA4335"
                  />
                </svg>
                <div className="text-left">
                  <div className="text-sm font-bold">개인 사용자</div>
                  <div className="text-xs text-gray-500 font-normal">Google 계정으로 시작하기</div>
                </div>
              </button>

              {/* 기관 / 코치 카드 */}
              <button
                onClick={() => signIn('google', { callbackUrl: '/coach-verify' })}
                className="w-full py-4 px-4 rounded-xl bg-violet-600 text-white font-bold hover:bg-violet-500 transition-colors flex items-center gap-3"
              >
                <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
                  <circle cx="9" cy="7" r="4" />
                  <path d="M23 21v-2a4 4 0 0 0-3-3.87" />
                  <path d="M16 3.13a4 4 0 0 1 0 7.75" />
                </svg>
                <div className="text-left">
                  <div className="text-sm font-bold">기관 / 코치</div>
                  <div className="text-xs text-white/60 font-normal">사전 등록된 이메일로 접속</div>
                </div>
              </button>
            </div>

          </div>
        </motion.div>
      </div>
    </main>
  )
}
